...
export const metadata = {
    title: MONEY_CONTENT.tax.title,
    description: MONEY_CONTENT.tax.description,
    alternates: { canonical: 'https://info.stac100.com/money/tax' }
};

export default async function TaxPage() {
    const data = MONEY_CONTENT.tax;
    const allNews = await getLatestNews();

    // Filter news related to Tax
    const relatedNews = allNews.filter(n =>
        n.title.includes('세금') || n.tags.includes('절세') || n.tags.includes('주식')
    ).slice(0, 8);
    ...
    return (
        <main className={styles.container}>
            ...
            <AdSenseSlot id="money-tax-top" />

            {/* Concept Intro */}
            <section style={{ background: '#1e1e1e', padding: '30px', borderRadius: '12px', marginBottom: '40px' }}>
                <h2 style={{ color: '#00dbbd', marginBottom: '20px' }}>💡 세금 투자의 핵심</h2>
                {data.intro.map((p, i) => (
                    <p key={i} style={{ marginBottom: '10px', color: '#ddd' }}>• {p}</p>
                ))}
            </section>

            <AdSenseSlot id="money-tax-mid" />

            {/* [New] Long-tail Page Links */}
            <section style={{ marginBottom: '50px' }}>
                <h2 className={styles.blockTitle}>📚 세금 심화 가이드 (Tax Guides)</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '15px' }}>
                    {Object.entries(MONEY_LONGTAIL_DATA.tax || {}).map(([slug, item]) => (
                        <Link key={slug} href={`/money/tax/${slug}`} style={{ background: '#222', padding: '20px', borderRadius: '12px', border: '1px solid #333', display: 'block' }}>
            ...
            <section style={{ marginBottom: '50px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h2 className={styles.blockTitle}>📰 세금 최신 뉴스</h2>
                </div>
            ...
            <AdSenseSlot id="money-tax-bottom" />
        </main>
    );
}
