...
export const metadata = {
    title: MONEY_CONTENT.us_stocks.title,
    description: MONEY_CONTENT.us_stocks.description,
    alternates: { canonical: 'https://info.stac100.com/money/us-stocks' }
};

export default async function USStocksPage() {
    const data = MONEY_CONTENT.us_stocks;
    const allNews = await getLatestNews();

    // Filter news related to US Stocks
    const relatedNews = allNews.filter(n => n.category === 'Overseas').slice(0, 8);
    ...
    return (
        <main className={styles.container}>
            ...
            <AdSenseSlot id="money-us-top" />

            {/* Concept Intro */}
            <section style={{ background: '#1e1e1e', padding: '30px', borderRadius: '12px', marginBottom: '40px' }}>
                <h2 style={{ color: '#00dbbd', marginBottom: '20px' }}>💡 서학개미 필독 체크포인트</h2>
                {data.intro.map((p, i) => (
            ...
            <section style={{ marginBottom: '50px' }}>
                <h2 className={styles.blockTitle}>🇺🇸 인기 미국 주식 바로가기</h2>
                <div className={styles.stockHubGrid}>
                    {Object.entries(STOCK_DATA).filter(([k, v]) => v.market === 'overseas').map(([key, stock]) => (
            ...
            <AdSenseSlot id="money-us-mid" />

            {/* [New] Long-tail Page Links */}
            <section style={{ marginBottom: '50px' }}>
                <h2 className={styles.blockTitle}>📚 미국주식 투자 심화 가이드</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '15px' }}>
                    {Object.entries(MONEY_LONGTAIL_DATA['us-stocks'] || {}).map(([slug, item]) => (
                        <Link key={slug} href={`/money/us-stocks/${slug}`} style={{ background: '#222', padding: '20px', borderRadius: '12px', border: '1px solid #333', display: 'block' }}>
            ...
            <section style={{ marginBottom: '50px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h2 className={styles.blockTitle}>🗽 미국 증시 실시간 속보</h2>
                </div>
            ...
            {/* FAQ Section */}
            <section style={{ marginBottom: '50px' }}>
                <h2 className={styles.blockTitle}>🙋‍♂️ 미국주식 세금/시간 FAQ</h2>
            ...
            <AdSenseSlot id="money-us-bottom" />
        </main>
    );
}
