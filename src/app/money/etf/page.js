...
export const metadata = {
    title: MONEY_CONTENT.etf.title,
    description: MONEY_CONTENT.etf.description,
    alternates: { canonical: 'https://info.stac100.com/money/etf' }
};

export default async function ETFPage() {
    const data = MONEY_CONTENT.etf;
    const allNews = await getLatestNews();

    // Filter news related to ETF/Semicon (Simulated logic)
    const relatedNews = allNews.filter(n =>
        n.title.includes('ETF') || n.tags.includes('반도체') || n.tags.includes('2차전지')
    ).slice(0, 8);
    ...
    return (
        <main className={styles.container}>
            ...
            <AdSenseSlot id="money-etf-top" />

            {/* Concept Intro */}
            <section style={{ background: '#1e1e1e', padding: '30px', borderRadius: '12px', marginBottom: '40px' }}>
                <h2 style={{ color: '#00dbbd', marginBottom: '20px' }}>💡 ETF 투자의 핵심</h2>
                {data.intro.map((p, i) => (
                    <p key={i} style={{ marginBottom: '10px', color: '#ddd' }}>• {p}</p>
                ))}
            </section>

            {/* Popular ETFs (Mock Data mixed with Stocks) */}
            <section style={{ marginBottom: '50px' }}>
                <h2 className={styles.blockTitle}>🔥 주목할 만한 섹터 ETF & 종목</h2>
                <div className={styles.stockHubGrid}>
                    {/* Using Stock Cards as proxies for Sector Leaders */}
                    {Object.entries(STOCK_DATA).slice(0, 4).map(([key, stock]) => (
                        <Link key={key} href={`/stock/${key}`} className={styles.stockHubCard}>
                            <span className={styles.stockName}>{stock.sector} 대장주</span>
                    ...
                    <div className={styles.stockHubCard} style={{ border: '1px dashed #444', justifyContent: 'center' }}>
                        <span style={{ color: '#888' }}>TIGER 미국테크TOP10</span>
                    </div>
                    <div className={styles.stockHubCard} style={{ border: '1px dashed #444', justifyContent: 'center' }}>
                        <span style={{ color: '#888' }}>KODEX 2차전지산업</span>
                    </div>
                </div>
            </section>

            <AdSenseSlot id="money-etf-mid" />
            ...
            <section style={{ marginBottom: '50px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h2 className={styles.blockTitle}>📰 ETF & 섹터 최신 뉴스</h2>
                </div>
            ...
            <AdSenseSlot id="money-etf-bottom" />
        </main>
    );
}
